...
import com.matheusvargas481.cloudnative.tema1.AppConfig;
...
public class CalculadoraServiceTest {
    ...
    public void testCalculoDaSoma() {
        assertEquals(7.0, calculadoraService.novoCalculo("+", 5D, 2D), 0);
    }

    @Test
    public void testCalculoDaSubtracao() {
        assertEquals(5D, calculadoraService.novoCalculo("-", 10D, 5D), 0);
    }

    @Test
    public void testCalculoDaMultiplicacao() { assertEquals(100D, calculadoraService.novoCalculo("*", 10D, 10D), 0d); }

    @Test
    public void testCalculoDaDivisao() { assertEquals(10D, calculadoraService.novoCalculo("/", 100D, 10D), 0); }

    @Test(expected = DivisaoPorZeroNaoExisteException.class)
    public void testExceptionDoCalculoDaDivisaoPorZero() {
        calculadoraService.novoCalculo("/", 10D, 0D);
    }

    @Test
    public void testCalculoDaPotencia() {
        assertEquals(32D, calculadoraService.novoCalculo("^", 2D, 5D), 0);
    }

    @Test
    public void testHistorico() {
        calculadoraService.novoCalculo("+", 5D, 5D);
        calculadoraService.novoCalculo("-", 5D, 5D);
        calculadoraService.novoCalculo("*", 5D, 5D);
        calculadoraService.novoCalculo("/", 5D, 5D);
        calculadoraService.novoCalculo("^", 5D, 5D);
